...
func (t *TradesDB) DateOf(number int64) (int64, error) {
	...
	if err != nil {
		return 0, util.Err("*TradesDB.DateOf(): ", err)
	}
...
func (t *TradesDB) MaxTidBeforeDate(date int64) (int64, error) {
	...
	if err != nil {
		return 0, util.Err("*TradesDB.MaxTidBeforeDate(): ", err)
	}
	if date < firstDate {
		msg := fmt.Sprintf("*TradesDB.MaxTidBeforeDate(): %s中最老的数据，在%s之后才产生。", t.Name(), util.DateOf(date))
		return 0, errors.New(msg)
	}
	...
	if lastDate < date {
		msg := fmt.Sprintf("*TradesDB.MaxTidBeforeDate(): %s中最新的数据，在%s之前。", t.Name(), util.DateOf(date))
		return 0, errors.New(msg)
	}
